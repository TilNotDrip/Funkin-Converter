package funkin.converter.util.macro;
...
class ClassMacro
{
	...
	public static macro function listClassesInPackage(targetPackage:String, includeSubPackages:Bool = true):ExprOf<Iterable<Class<Dynamic>>>
	{
		...
		return macro funkin.converter.util.macro.CompiledClassList.get($v{request});
	}
	...
	public static macro function listSubclassesOf<T>(targetClassExpr:ExprOf<Class<T>>):ExprOf<List<Class<T>>>
	{
		...
		return macro funkin.converter.util.macro.CompiledClassList.getTyped($v{request}, ${targetClassExpr});
	}
	...
	{
		var compiledClassList:ClassType = MacroUtil.getClassType("funkin.converter.util.macro.CompiledClassList");
